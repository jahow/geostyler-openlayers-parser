...
describe('OlFlatStyleParser implements StyleParser', () => {
  ...
  describe('#writeStyle', () => {
    ...
    it('can write an OpenLayers Flat LineSymbolizer', async () => {
      const { output: geoStylerStyle } = await styleParser.writeStyle(line_simpleline);
      expect(geoStylerStyle).toBeDefined();
      expect(geoStylerStyle).toEqual(flat_line_simpleline);
    });
    it('can write an OpenLayers Flat PolygonSymbolizer', async () => {
      const { output: geoStylerStyle } = await styleParser.writeStyle(polygon_transparentpolygon);
      expect(geoStylerStyle).toBeDefined();
      expect(geoStylerStyle).toEqual(flat_polygon_transparentpolygon);
    });
